...
test_that("lsm_p_area is typestable", {
    expect_s3_class(lsm_p_area(landscape), "tbl_df")
    expect_s3_class(lsm_p_area(landscape_stack), "tbl_df")
    expect_s3_class(lsm_p_area(landscape_list), "tbl_df")
})
...
test_that("lsm_p_area equals FRAGSTATS", {
    lsm_landscape <- lsm_p_area(landscape) |> dplyr::pull(value)
    lsm_augusta <- lsm_p_area(augusta_nlcd) |> dplyr::pull(value)

    fs_landscape <- dplyr::filter(fragstats_patch, LID == "landscape", metric == "area") |> dplyr::pull(value)
    fs_augusta <- dplyr::filter(fragstats_patch, LID == "augusta_nlcd", metric == "area") |> dplyr::pull(value)

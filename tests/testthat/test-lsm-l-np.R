...
test_that("lsm_l_np equals FRAGSTATS", {
    lsm_landscape <- lsm_l_np(landscape) |> dplyr::pull(value)
    lsm_augusta <- lsm_l_np(augusta_nlcd) |> dplyr::pull(value)

    fs_landscape <- dplyr::filter(fragstats_landscape, LID == "landscape", metric == "np") |> dplyr::pull(value)
    fs_augusta <- dplyr::filter(fragstats_landscape, LID == "augusta_nlcd", metric == "np") |> dplyr::pull(value)
